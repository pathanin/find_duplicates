...
- **`/` and `/static/{path}` must keep their `Cache-Control: no-cache` (`NO_CACHE`).** Those paths carry no version, so an upgraded install serves new bytes at the same URL. With only `Last-Modified` a browser applies heuristic freshness — Chrome served a week-old `app.js` from cache on a normal navigation (`transferSize: 0`) for hours, which surfaces as new markup driven by a stale script: the button is there and clicking it does nothing. The `ETag` keeps the forced revalidation a 304. `tests/test_web_api.py` locks the headers.
