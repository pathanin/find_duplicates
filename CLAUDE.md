...
- `load_hash_gray`'s reduced-decode and full-decode paths must agree on the **64-bit** hash bits — check `MIN_REDUCED_DECODE_SIDE` before touching either one. They do *not* agree on the 256-bit confirmation half: measured drift is up to 16 bits there, which is expected and already absorbed by `CONFIRM_HASH_THRESHOLD`. Don't chase it as a bug.
